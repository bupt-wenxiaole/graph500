...
SOATTR int aml_init( int *argc, char ***argv ) {
	...
	sprintf(edgesfilepath, "/mnt/nfs/xwen/liveJournal_vc_part%d/G.%d",num_procs, myproc);
	subgraphedgesfile = fopen(edgesfilepath, "w");
	char isolatedpath[100];
	sprintf(isolatedpath, "/mnt/nfs/xwen/liveJournal_vc_part%d/Isolateds.%d",num_procs, myproc);
	isolatedfile = fopen(isolatedpath, "w");
	char masterpath[100];
	sprintf(masterpath, "/mnt/nfs/xwen/liveJournal_vc_part%d/Master.%d",num_procs, myproc);
	masterfile = fopen(masterpath, "w");
	char mirrorpath[100];
	sprintf(mirrorpath, "/mnt/nfs/xwen/liveJournal_vc_part%d/Mirror.%d",num_procs, myproc);
	mirrorfile = fopen(mirrorpath, "w");
#ifdef DUMP_THE_DEGREE
	char degreefilepath[100];
	sprintf(degreefilepath, "/mnt/nfs/xwen/liveJournal_vc_part%d/Degree.%d",num_procs, myproc);
